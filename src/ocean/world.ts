...
export class OceanWorld {
  ...
  private original=new THREE.Group();private beyond!:BeyondWorld;private portal=createPortal(oceanGate);private portalCooldown=0;
  ...
  command(c:Command){
    if(c==='gate'){const gate=this.state.realm==='ocean'?oceanGate:returnGate;this.state.x=gate.x+Math.sin(gate.rotation)*12;this.state.z=gate.z+Math.cos(gate.rotation)*12;this.velocity.set(0,0);this.target=null;this.state.race=-1;this.rings.forEach(r=>r.visible=false);this.message(this.state.realm==='beyond'?'Swim through the violet veil to return.':echoRewards(this.state.pearls.length).portal?'The veil is open. Swim through the violet wall.':`The veil needs ${18-this.state.pearls.length} more echoes.`);return;}
