...
export function createWater(){
 ...
 const uniforms={time:{value:0},night:{value:1},pigment:{value:pigment},islands:{value:places.map(p=>new THREE.Vector3(p.x,p.z,p.radius))}};
 ...
 vec3 deep=mix(vec3(.047,.26,.36),vec3(.035,.11,.25),night);vec3 shallow=mix(vec3(.21,.51,.49),vec3(.12,.34,.43),night);vec3 c=mix(shallow,deep,smoothstep(-1.,30.,d));c*=.84+broad*.34;c+=vec3(.13,.21,.26)*(brush-.34)*.37;
 ...
 c=mix(c,vec3(.22,.35,.49),fresnel*.27);
 float fog=1.-exp(-.0044*.0044*pow(gl_FragCoord.z/gl_FragCoord.w,2.));c=mix(c,vec3(.12,.22,.36),clamp(fog,0.,.88));gl_FragColor=vec4(c,1.);
 }`});
